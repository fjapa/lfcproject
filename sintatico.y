...
PROGRAMA: DECLARACAO_LISTA{$$ = producao_programa_declaracaoLista($1);}
;
...
VAR_DECLARACAO:
TIPO_ESPECIFICADOR ID PONTOVIRGULA{$$ = producao_vardeclaracao_tipoEspecificador_id($1, $2);}
| TIPO_ESPECIFICADOR ID ABRECOLCHETE DIGITO FECHACOLCHETE PONTOVIRGULA{$$ = producao_vardeclaracao_tipoEspecificador_id_aC_NUM_fC($1, $2, $4);}
;

/*(05) tipo-especificador -> int | void*/ //String adicionada à linguagem C-
TIPO_ESPECIFICADOR:
INT{$$ = producao_tipoEspecificador_int();}
| VOID{$$ = producao_tipoEspecificador_void();}
| STRING{$$ = producao_tipoEspecificador_string();}
;

/*(06) fun-declaração —> tipo-especificador ID ( params ) composto-decl*/
FUN_DECLARACAO:
TIPO_ESPECIFICADOR ID ABREPARENTESES PARAMS FECHAPARENTESES COMPOSTO_DECL{$$ = producao_tipoEspecificador_ID_aP_params_fP_compostoDecl($1, $2, $4, $6);}
;
...
PARAM:
TIPO_ESPECIFICADOR ID{$$ = producao_param_tipoEspecificador_ID($1, $2);}
| TIPO_ESPECIFICADOR ID ABRECOLCHETE FECHACOLCHETE{$$ = producao_param_tipoEspecificador_ID_aC_fC($1, $2);}
;
...
VAR:
ID{$$ = producao_var_id($1);}
| ID ABRECOLCHETE EXPRESSAO FECHACOLCHETE{$$ = producao_var_id_aC_exp_fC($1, $3);}
;
...
/*(21) relacional -> <= | < | > | >= | == | != */
RELACIONAL:
MENORIGUAL{$$ = producao_relacional_menorIgual();}
| MENOR{$$ = producao_relacional_menor();}
| MAIOR{$$ = producao_relacional_maior();}
| MAIORIGUAL{$$ = producao_relacional_maiorIgual();}
| IGUALIGUAL{$$ = producao_relacional_igualIgual();}
| DIFERENTE{$$ = producao_relacional_diferente();}
;
...
/*(23) soma —> + | - */
SOMA:
MAIS{$$ = producao_soma_mais();}
| MENOS{$$ = producao_soma_menos();}
;
...
/*(25) mult -> * | / */
MULT:
ASTERISCO{$$ = producao_mult_asterisco();}
| BARRA{$$ = producao_mult_aBarra();}
;
...
/*(27) ativação —> ID ( args ) */
ATIVACAO:
ID ABREPARENTESES ARGS FECHAPARENTESES{$$ = producao_ativacao_ID_aP_args_fP($1, $3);}
;
...
COMANDOS:
  ...
  bison -v -d -o sintatico_bison.c sintatico.y

  flex -v -t lexico.l > lexico_yacc.c

  gcc sintatico_bison.c lexico_yacc.c -o Compilador
